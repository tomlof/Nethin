...
class BaseModel(tf.keras.Model):
    ...
    name : str, optional
        The name of the network. Default is ``None``, no name.
    """
...
class FullyConnectedNetwork(BaseModel):
    """Generates a standard fully connected, or dense architechture.

    The architecture has Dense + ReLU layers, with optional dropout and batch
    normalisation.

    Parameters
    ----------
    input_shape : int
        The size of the input data, excluding the batch dimension.

    num_filters : tuple of int, optional
        The number of neurons in each dense layer. Default is
        ``(64, 64)``, i.e. two dense layers with 64 neurons in each.

    activations : str, tuple of str, or Activation, optional
        ...
        then it must have the same length as ``num_filters``. Default is
        ``"relu"``.

    kernel_regularizer : str, Regularizer or Callable, optional
        Regularizer function applied to the dense weights.

    bias_regularizer : str, Regularizer or Callable, optional
        Regularizer function applied to the dense bias vectors.
    ...
    use_batch_normalization : bool, optional
        Whether or not to use batch normalization after each layer. Default is
    ...
    >>>
    >>> model = nethin.models.FullyConnectedNetwork(
    ...     input_shape,
    ...
    def __init__(self,
        ...
        self.num_neurons = tuple(num_neurons)
        if len(self.num_neurons) < 1:
            raise ValueError("``num_neurons`` must have length at least 1.")
        ...
        if isinstance(activations, tf.keras.layers.Activation):
            warnings.warn("The activation is reused in each layer."
                          "Pass a tuple with the activations for each "
                          "layer instead, if you need them to be independent.")
            activations = (activations,)
        activations = tuple(activations)
        if len(activations) != len(self.num_neurons):
        ...
        if isinstance(dropout_rate, (int, float)):
            do = [max(0.0, min(float(dropout_rate), 1.0))
                  for i in range(len(self.num_neurons))]
            dropout_rate = tuple(do)
        if len(dropout_rate) != len(self.num_neurons):
    ...
    def build(self, input_shape):
        ...
        # self._activation = [None] * len(self.num_neurons)
        self._dropout = [None] * len(self.num_neurons)
        for i in range(len(self.num_neurons)):

            num_neurons_i = self.num_neurons[i]
            # activation_i = self.__activations[i]
            ...
            if self.use_batch_normalization:
                ...
                bn = None
            self._batchnorm[i] = bn

            # Note: The activations are already created!
            # self.__activation[i] = activation_i

            # Don't apply dropout to the last layer
            if dropout_rate_i > 0.0 and (i < len(self.num_neurons) - 1):
                do = tf.keras.layers.Dropout(rate=dropout_rate_i)
            else:
                do = None
            self._dropout[i] = do
    ...
    def call(self, inputs, training=False):
        ...
        for i in range(len(self.num_neurons)):
            ...
            if self._batchnorm[i] is not None:
                x = self._batchnorm[i](x)

            if self._activations[i] is not None:
                x = self._activations[i](x)

            if (self._dropout[i] is not None) and training:
                x = self._dropout[i](x)
...
class ConvolutionalNetwork(BaseModel):
    ...
    num_filters : tuple of int, optional
        ...
        ``(64, 64, 64, 64, 64)``, i.e. five layers with 64 filters.
    ...
    dense_sizes : tuple of int, optional
        The number of dense layer neurons following the convolution. Default is
    ...
    def __init__(self,
        ...
        super(ConvolutionalNetwork, self).__init__(input_shape,
                                                   data_format=data_format,
                                                   name=name)
        ...
        if isinstance(activations, tf.keras.layers.Activation):
            warnings.warn("The activation is reused in each layer."
                          "Pass a tuple with the activations for each "
                          "layer instead, if you want them to be independent.")
            activations = (activations,)
        activations = tuple(activations)
        if len(activations) != len(self.num_filters):
            raise ValueError("``activations`` should have the same length as "
                             "``num_conv_layers``.")
        if isinstance(activations, (list, tuple)):
            _activations = [None] * len(activations)
        ...
        if isinstance(dense_activations, tf.keras.layers.Activation):
            warnings.warn("The dense activation is reused in each dense layer."
                          "Pass a tuple with the dense activations for each "
                          "dense layer instead, if you want them to be "
                          "independent.")
            dense_activations = (dense_activations,)
        dense_activations = tuple(dense_activations)
        if len(dense_activations) != len(self.dense_sizes):
            raise ValueError("``dense_activations`` should have the same "
                             "length as `dense_sizes``.")
        if isinstance(dense_activations, (list, tuple)):
            _activations = [None] * len(dense_activations)
            for i in range(len(dense_activations)):
                if isinstance(dense_activations[i], str) \
                        or dense_activations[i] is None:
                    _activations[i] = tf.keras.layers.Activation(
                        dense_activations[i])
                else:
                    _activations[i] = dense_activations[i]
            self._dense_activations = tuple(_activations)
        else:
        ...
        if isinstance(dropout_rate, (int, float)):
            do = [max(0.0, min(float(dropout_rate), 1.0))
                  for i in range(len(self.dense_sizes))]
            dropout_rate = tuple(do)
        if len(dropout_rate) != len(self.dense_sizes):
            raise ValueError("``dropout_rate`` should have the same length as "
                             "``dense_sizes``.")
        if isinstance(dropout_rate, (list, tuple)):
            do = [max(0.0, min(float(dropout_rate[i]), 1.0))
        ...
        # Create the network's layers
        self._conv = [None] * len(self.num_filters)
        self._batchnorm = [None] * len(self.num_filters)
        # self._activation = [None] * len(self.num_filters)
        self._subsample = [None] * len(self.num_filters)
        self._flatten = None
        self._dense = [None] * len(self.dense_sizes)
        self._dense_activation = [None] * len(self.dense_sizes)
        self._dropout = [None] * len(self.dense_sizes)
        for i in range(len(self.num_filters)):
            ...
            if self._input_dim == 1:
                ...
                conv = tf.keras.layers.Convolution3D(
                        num_filters_i,
            ...
            if subsample_i:
                if self.use_maxpool:
                    if self._input_dim == 1:
                        subsample = tf.keras.layers.MaxPooling1D(
                                pool_size=2,
                                strides=2,
                                padding="same",
                                data_format=self.data_format)
                    ...
                    if self._input_dim == 1:
                        ...
                        subsample = tf.keras.layers.Convolution2D(
                                num_filters_i,
                                (filter_size_i, filter_size_i),
                                strides=(2, 2),
                                padding="same",
                        ...
                        subsample = tf.keras.layers.Convolution3D(
                                num_filters_i,
                                (filter_size_i, filter_size_i, filter_size_i),
                                strides=(2, 2, 2),
                                padding="same",
        ...
        if len(self.dense_sizes) > 0:
            ...
            for i in range(len(self.dense_sizes)):
                ...
                # activation_i = self._dense_activations[i]
                dropout_rate_i = self.dropout_rate[i]
                ...
                # else:
                #     activation = None
                # self._dense_activation[i] = activation
    ...
    def call(self, inputs, training=False):
        ...
        for i in range(len(self.num_filters)):
            ...
            if self._subsample[i] is not None:
                x = self._subsample[i](x)
        ...
        for i in range(len(self.dense_sizes)):
            ...
            if self._dense_activations[i] is not None:
                x = self._dense_activations[i](x)
